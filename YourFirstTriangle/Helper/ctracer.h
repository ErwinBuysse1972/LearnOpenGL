...
std::ostream& operator<<(std::ostream& os, const log_watch<Precision, Clock>& lw) {
    ...
    tm tm;
    localtime_s(&tm, &t);
    os << std::put_time(&tm, lw.m_format.c_str());
...
public:
    ...
    virtual void Write(const char* /*data*/, TracerLevel /*lvl = TracerLevel::TRACER_DEBUG_LEVEL*/){}
    virtual void WriteBinData(const char* /*binData*/, unsigned long /*dwLen*/, bool /*bRawNoAsci*/){}
};
