    ...
    try
    {
        ...
        if (_level <= TracerLevel::TRACER_DEBUG_LEVEL)
        {
            ...
            if (!m_bUseBinDataWriting)
                ...
                WriteBinData(outStr.c_str(), (unsigned long)outStr.length(), true);
        }
    }
    catch(...)
    {
...
unsigned long CFileTracer::GetFileSize(void)
{
    ...
    try
    {
        if (m_out)
        {
            ...
            begin = (unsigned long)m_out->tellp();
            m_out->seekp(0, ios::end);
            end = (unsigned long)m_out->tellp();
    ...
    try
    {
        ...
        for (int i = 0; i < (int)dwLen; i++)
        {
            if (bRawNoAscii)
            {
                if (binData[i] < 0x20)
                {
                    n += sprintf_s(buf.get() + n, (size - n),"%02x.", binData[i]);
                }
                else
                {
                    n += sprintf_s(buf.get()+n, (size - n), "%c", binData[i]);
                }
            }
            else
            {
