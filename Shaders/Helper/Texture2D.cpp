...
// Constructor
//-----------------------------------------------------------------------------
Texture2D::Texture2D(std::shared_ptr<CTracer> tracer)
    : m_trace(tracer)
    , m_Texture(0)
{
}
...
bool Texture2D::loadTexture(const string& fileName, bool generateMipMaps)
{
    ...
    glBindTexture(GL_TEXTURE_2D, m_Texture); // all upcoming GL_TEXTURE_2D operations will affect our texture object (mTexture)
