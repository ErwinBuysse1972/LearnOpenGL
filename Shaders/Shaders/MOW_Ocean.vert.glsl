...
void main()
{
    ...
    vec4 vlNormal = vec4(Normal, 1.0);
    vec4 light0Ambient = VP.m_vrLight_Ambient_0;
    vec4 light0Diffuse = VP.m_vrLight_Diffuse_0;
    vec3 SkyLightFogParams = vec3(0.0, 0.0, 0.0);
    vec4 skylightLight = vec4(0.0, 0.0, 0.0, 1.0);
    vec4 outColor0 = vec4(0.0, 0.0, 0.0, 1.0);
    vec4 materialDiffuse = VP.m_vrMaterial_Diffuse_Front;
    ...
    vec4 vEyePos = ModelViewMatrix * vec4(inPosition, 1.0);
